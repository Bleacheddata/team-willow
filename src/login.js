...
class App extends Component {
  ...
  componentDidMount() {
    ...
    PubSub.subscribe(UserSearchFood, (msg, data) => {
      ...
      getFoodList(data.keyword).then(function (result) {
        if (result.success) {
          parent.setState({
            searchKeyword: data.keyword,
    ...
    PubSub.subscribe(
      UserAddedFoodToIntakeList,
      async (msg, { foodId, name, calories, photo, quantity, intakeDate }) => {
        const newFood = await addFood(this.state.user.uid, {
          foodId,
          name,
          calories,
          photo,
    ...
    PubSub.subscribe(
      UserUpdatedFoodFromIntakeList,
      async (msg, { foodId, quantity, intakeDate }) => {
        const foodToUpdate = parent.state.intakeFood.find(
          (f) =>
            f.foodId === foodId &&
            f.intakeDate.toDateString() === intakeDate.toDateString()
        );
        await updateFood(
          this.state.user.uid,
          foodToUpdate.id,
    ...
    PubSub.subscribe(
      UserDeletedFoodFromIntakeList,
      async (msg, { foodId, intakeDate }) => {
        const foodToDelete = parent.state.intakeFood.find(
          (f) =>
            f.foodId === foodId &&
            f.intakeDate.toDateString() === intakeDate.toDateString()
        );
  ...
  render() {
    if (this.state.user.isLogin) {
      const layouts = {
        ...
        intake: <IntakeView intakeFood={this.state.intakeFood} />,
        home: <StatisticView intakeFood={this.state.intakeFood} />
      };
      return (
        <div className="App">
          <Box sx={{ pb: 7 }}>
