const express = require("express");

// recordRoutes is an instance of the express router.
// We use it to define our routes.
// The router will be added as a middleware and will take control of requests starting with path /record.
const routes = express.Router();


// This will help us connect to the database
const dbo = require("../db/conn");

// This help convert the id from string to ObjectId for the _id.
const ObjectId = require("mongodb").ObjectId;


function randomIntegerBetween(min, max) {
  min = Math.ceil(min);
  max = Math.floor(max);
  return Math.floor(Math.random() * (max - min + 1) + min); //The maximum is inclusive and the minimum is inclusive
}
function randomInteger(max) {
    return Math.floor(Math.random() * max);
  }
  
//   let cardCollection = [
//     {
//       name: "Raiden",
//       image:
//         "https://i.pinimg.com/originals/71/ff/e2/71ffe2e22795d6ca9b80da4e9fd154c8.jpg"
//     },
//     {
//       name: "Phoenix",
//       image:
//         "https://i.pinimg.com/originals/4e/43/09/4e4309728598bfc1b06d6681e38502ff.jpg"
//     },
//     {
//       name: "Ghost",
//       image:
//         "https://i.pinimg.com/originals/6b/f4/da/6bf4da99007d304f76d46ff8e7f7ad46.jpg"
//     },
//     {
//       name: "Nightrunner",
//       image:
//         "https://i.pinimg.com/736x/18/ed/74/18ed74b95a0c3856087ff62b9de76175--cyberpunk-character-shooter-games.jpg"
//     },
//     {
//       name: "Archangel",
//       image:
//         "https://i.pinimg.com/originals/7f/17/4d/7f174d7f31ac58a40ef4447bf78af93b.jpg"
//     },
//     {
//       name: "Scorch",
//       image:
//         "https://i.pinimg.com/originals/4f/9c/ef/4f9cef22478edd093eb6a74182758c6c.jpg"
//     },
//     {
//       name: "Tempest",
//       image:
//         "https://i.pinimg.com/originals/c6/a8/9d/c6a89d990f0055ec4035db19c0df0668.jpg"
//     },
//     {
//       name: "Phantom",
//       image:
//         "https://i.pinimg.com/originals/52/e4/88/52e48892c471b8575b71b2ec81d21538.jpg"
//     },
//     {
//       name: "Viper",
//       image:
//         "https://i.pinimg.com/736x/cd/3e/2b/cd3e2bffa7f01d4b9ab8fc4a016bef93.jpg"
//     },
//     {
//       name: "Nomad",
//       image:
//         "https://i.pinimg.com/originals/50/fa/c7/50fac7e383458e9360faa499f27819d1.jpg"
//     }
//   ];
 
  
// function createCard (){
//     let cardType = ["Depletor", "Generator"];
//     let cardClass = ["None", "None", "None", "None", "None", "None", "None", "None", "None", "None", "None", "None", "None", "None", "None"];
//     let card = cardCollection[randomInteger(cardCollection.length)];
//     card.power = randomIntegerBetween(1,3);
//     card.class = cardClass[randomInteger(cardClass.length)];

   
  
//     if (card.power >= 1 && card.power <= 3 ) {
//       card.rarity = "Common";
     
//     }
//     else if (card.power >= 4 && card.power <= 6) {
//       card.rarity = "Uncommon";
      
//     }
//     else if (card.power >= 7 && card.power <= 9) {
//       card.rarity = "Rare";
  
//     }
//     else if ((card.power >= 10 && card.power <= 12)) {
//       card.rarity = "Ultra-Rare";

//     }
  
//     card.type = cardType[randomInteger(cardType.length)];
  
//     if (card.type === "Depletor") {
//       card.power = card.power * -1;
//     }
//     return card;
//   }
  
// This section will help you get a list of all the records.

routes.route("/cardpack").get(function (req, res) {
    let db_connect = dbo.getDb("TeamWillow");
    db_connect
        .collection("cards")
        .find({})
        .toArray(function (err, result) {
            if (err) throw err;
   


            let commonFiltered = result.filter(function(element){ 
              return  element.rarity == "Common";
          });
            let uncommonFiltered = result.filter(function(element){ 
              return  element.rarity == "Uncommon";
          });
          let rareFiltered = result.filter(function(element){ 
            return  element.rarity == "Rare";
        });

        let ultraFiltered = result.filter(function(element){ 
          return  element.rarity == "Ultra-Rare";
      });

    
            uncommonFiltered.splice(0, 20);
            rareFiltered.splice(0, 30);
            ultraFiltered.splice(0,38)

            let cardsToPickFrom = [...commonFiltered, ...uncommonFiltered, ...rareFiltered, ...ultraFiltered];
            
            let cardPack = shuffle(cardsToPickFrom);

            cardPack.splice(0, 67);
            
            res.json(cardPack);
           
        });

   
    

       
  
});
function shuffle(array) {
  let curId = array.length;
  // There remain elements to shuffle
  while (0 !== curId) {
    // Pick a remaining element
    let randId = Math.floor(Math.random() * curId);
    curId -= 1;
    // Swap it with the current element.
    let tmp = array[curId];
    array[curId] = array[randId];
    array[randId] = tmp;
  }
  return array;
}

module.exports = routes;